...
class APIKeyManager(object):
    ...
    def _update_key_manager_file(self):
        ...
        with open(filename, "w") as key_manager_file:
            for username in self.key_manager:
                newline = username + "\t" + self.key_manager[username]['user_id'] + "\t" + \
                          self.key_manager[username]['pwd_salt'] + "\t" + self.key_manager[username]['hashed_pwd']
    ...
    def _encrypted_file_and_pwd_to_api_keys(self, username):
        ...
        if success:
            ...
            for salt in self.key_manager[username]['aes_key_salts']:
                ...
                filename = self.self_path + "/key_manager/" +  user_id_code + "." + salt_code + "." + \
                           str(self.key_manager[username]['aes_key_salts'].index(salt)) + ".txt"
                with open(filename, "r") as aes_file:
                    encrypted_data = aes_file.read()
                    api_keys.append(black_box.passphrase_decrypt(base64.b64decode(encrypted_data), pwd, \
    ...
    def _pwd_and_api_keys_to_encrypted_file(self, username=None, api_keys = [None, None]):
        ...
        if username is not None:
            ...
            self.this_user['aes_key_salts'] = [base64.b64encode(os.urandom(black_box.salt_length)), \
                                               base64.b64encode(os.urandom(black_box.salt_length))]
            ...
            self.this_user['hashed_pwd'] = base64.b64encode(black_box.hash_passphrase(pwd, \
                                                                                      self.this_user['pwd_salt'], bits_to_read=32))
            ...
            encrypted_data = [base64.b64encode(black_box.passphrase_encrypt(plaintext_one, pwd, salt_one)), \
                              base64.b64encode(black_box.passphrase_encrypt(plaintext_two, pwd, salt_two))]
